...
protected:
    ...
    char **argv_;
    std::string name_;
    std::string get_args_ = "f:d:g:hk:su:";
...
public:
    CLBase(int argc, char **argv, std::string name = "") :
        ...
        AddHelpLine('d', "dbfile", "pmem file to store the graph");
    ...
    bool ParseArgs() {
        ...
        if (dbfilename_ == "") {
            std::cout << "No PMEM specified. (Use -h for help)" << std::endl;
            return false;
        }
        if (scale_ != -1)
            symmetrize_ = true;
    ...
    void virtual HandleArg(signed char opt, char *opt_arg) {
        switch (opt) {
            case 'f':
                ...
                break;
            case 'd':
                dbfilename_ = std::string(opt_arg);
                break;
            case 'g':
                scale_ = atoi(opt_arg);
                break;
...
public:
    CLDelta(int argc, char** argv, std::string name) : CLApp(argc, argv, name) {
        get_args_ += "l:";
        AddHelpLine('l', "l", "delta parameter", std::to_string(delta_));
    }
