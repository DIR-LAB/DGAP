...
int64_t BUStep(snap_t<dst_id_t>* snaph, pvector<vid_t> &parent, GapbsBitmap &front, GapbsBitmap &next) {
...
int64_t TDStep(snap_t<dst_id_t>* snaph, pvector<vid_t> &parent,
...
void BitmapToQueue(snap_t<dst_id_t>* snaph, const GapbsBitmap &bm,
                   SlidingQueue<vid_t> &queue) {
#pragma omp parallel
  {
    QueueBuffer<vid_t> lqueue(queue);
...
pvector<vid_t> InitParent(snap_t<dst_id_t>* snaph) {
  pvector<vid_t> parent(snaph->get_vcount());
#pragma omp parallel for
  for (vid_t n=0; n < snaph->get_vcount(); n++)
    parent[n] = snaph->get_degree_out(n) != 0 ? -snaph->get_degree_out(n) : -1;
  return parent;
}
...
//  int64_t edges_to_check = snaph->num_edges_directed();
  ...
  int64_t scout_count = snaph->get_degree_out(root);
  while (!queue.empty()) {
...
void PrintBFSStats(snap_t<dst_id_t>* snaph, const pvector<vid_t> &bfs_tree) {
  int64_t tree_size = 0;
  int64_t n_edges = 0;
//  for (vid_t n : snaph->get_vcount()) {
  for (vid_t n=0; n < snaph->get_vcount(); n++) {
    if (bfs_tree[n] >= 0) {
      n_edges += snaph->get_degree_out(n);
      tree_size++;
    }
  }
