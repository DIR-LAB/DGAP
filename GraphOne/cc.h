...
pvector<vid_t> run_cc(snap_t<dst_id_t>* snaph) {
...
void PrintCompStats(snap_t<dst_id_t>* snaph, const pvector<vid_t> &comp) {
  cout << endl;
  unordered_map<vid_t, vid_t> count;
  for (vid_t comp_i : comp)
    count[comp_i] += 1;
  int k = 5;
  vector<pair<vid_t, vid_t>> count_vector;
  count_vector.reserve(count.size());
  for (auto kvp : count)
    count_vector.push_back(kvp);
  vector<pair<vid_t, vid_t>> top_k = TopK(count_vector, k);
...
bool CCVerifier(snap_t<dst_id_t>* snaph, const pvector<vid_t> &comp) {
  unordered_map<vid_t, vid_t> label_to_source;
  for (vid_t n=0; n < snaph->get_vcount(); n++)
