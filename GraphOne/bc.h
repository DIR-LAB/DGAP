...
static void ParallelLoadDegrees(snap_t<dst_id_t>* snaph, pvector<int32_t> &degrees) {
#pragma omp parallel for schedule(dynamic, 16384)
  for (vid_t u = 0; u < snaph->get_vcount(); u++) {
    degrees[u] = snaph->get_degree_out(u);
  }
}
...
void PBFS(snap_t<dst_id_t>* snaph, vid_t source, pvector<vid_t> &path_counts,
...
pvector<ScoreT> run_bc(snap_t<dst_id_t>* snaph, index_t _edge_count, sid_t root, vid_t num_iters) {
//  Timer t;
//  t.Start();
...
void PrintTopScores(snap_t<dst_id_t>* snaph, const pvector<ScoreT> &scores) {
  vector<pair<vid_t, ScoreT>> score_pairs(snaph->get_vcount());
  for (vid_t n=0; n<snaph->get_vcount(); n+=1)
    score_pairs[n] = make_pair(n, scores[n]);
  int k = 5;
  vector<pair<ScoreT, vid_t>> top_k = TopK(score_pairs, k);
  for (auto kvp : top_k)
...
bool BCVerifier(snap_t<dst_id_t>* snaph, sid_t root, vid_t num_iters,
