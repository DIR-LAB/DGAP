...
//template <class T>
pvector<ScoreT> run_pr(snap_t<dst_id_t>* snaph, int max_iters, double epsilon = 0)
{
...
#pragma omp parallel for
    for (vid_t n=0; n < snaph->get_vcount(); n++)
      outgoing_contrib[n] = scores[n] / snaph->get_degree_out(n);
...
void PrintTopPRScores(snap_t<dst_id_t>* snaph, const pvector<ScoreT> &scores) {
