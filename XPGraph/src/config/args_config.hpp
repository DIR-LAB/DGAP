...
void print_usage() {
    ...
    help += " -q: The number of executions for each graph query algorithm. Default: 1.\n";

    help += " -t: Thread count for buffering and flushing. Default: 16.\n"; //Cores in your system - 1.\n";
    help += " -s: Sockets count. Default: 1.\n";
    ...
    help += " --source: Source vertex id used by bfs and bc. Default: 0.\n";
...
void args_config(int argc, const char ** argv) {
    ...
    source = get_option_int("--source", 0); // Source vertex id used by bfs and bc. Default: 0.
    ...
    QUERY = get_option_int("-q", 1); // The number of executions for each graph query algorithm.

    /* ---------------------------------------------------------------------- */
    // Basic arguments for system info
    THD_COUNT = get_option_int("-t", 16); // Thread count for buffering and flushing. Default: 16.
    NUM_SOCKETS = get_option_int("-s", 1); // Sockets count. Default: 1.
    ...
    NUMA_OPT = get_option_int("-numa", 0); // Implementation of numa optimization: 0 for closing NUMA optimization,
    ...
    PBLK_POOL_SIZE = get_option_long("--pblk_pools", 128) * GB; // Total persistent PMEM block pool size threshold in GigaByte. Default: 64.
    /* ---------------------------------------------------------------------- */
    // PMEM mempool size

    /* ---------------------------------------------------------------------- */
    // Other arguments config
    CLWB = get_option_int("--clwb", 1); // clwb = 1 for clwb proactively
...
void prinf_config(){
    ...
    cout<< "\t - source = " << source << " (source vertex-id used by bfs and bc.) " << endl;
