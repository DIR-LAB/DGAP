...
public:
    levelgraph_t(vid_t _nverts, vid_t _source, metrics &_m):nverts(_nverts), source(_source), m(_m){
    ...
    inline vid_t get_source_vertex(){ return source; }
    inline void set_out_edge_count(index_t _n_out_edges) { n_out_edges = _n_out_edges; }
    inline void set_in_edge_count(index_t _n_in_edges) { n_in_edges = _n_in_edges; }
    inline index_t get_out_edge_count(){ return n_out_edges; }
    inline index_t get_in_edge_count(){ return n_in_edges; }
    ...
    void count_and_print_edgeshard() {
      std::pair<index_t, index_t> e_counts = edge_shard->count_total_edges_and_print_edgesum_per_thread();
      assert(e_counts.first == e_counts.second && "there is a mismatch in the total in and out edge count!");
      set_in_edge_count(e_counts.first);
      set_out_edge_count(e_counts.second);
    }
