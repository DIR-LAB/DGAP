...
class edge_shard_t {
 ...
 public:
    ...
    void cleanup();
    std::pair<index_t, index_t> count_total_edges_and_print_edgesum_per_thread();
...
std::pair<index_t, index_t> edge_shard_t::count_total_edges_and_print_edgesum_per_thread() {
  std::cout << "edge_shard->count_total_edges_and_print_edgesum_per_thread: " << std::endl;
  ...
  return std::make_pair(in_sum, out_sum);
}
